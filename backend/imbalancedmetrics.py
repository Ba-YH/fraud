...
class ImBinaryMetric:
    ...
    def gmean(self):
        P = self.TP + self.FN
        N = self.TN + self.FP
        assert (P != 0), " the number of the positive class equal zero"
        assert (N != 0), " the number of the negtive class equal zero"
        return np.sqrt(((1.0 * self.TN) / N) * ((1.0 * self.TP) / P))
    ...
    def aucroc(self, y_preprob):
        y = self.y_true.copy()
        return roc_auc_score(y, np.array(y_preprob))

    def displaymetric(self):
        return str(self.gmean()) + "\t" + str(self.recall()) + "\t" + str(self.precision()) + "\t" + str(
            self.f1()) + "\t" + str(self.MCC())
